...
if(message.mentions.users.size === 0) return message.reply("Tu n'as pas mentionné d'utilisateur'").catch(console.error);
        ...
        message.reply("Votre report a bien été envoyé.")
            bot.users.get("519186194886688779").send(`${userreport.id} ${userreport.username} à été report sur le serveur ${message.guild.name} par ${message.author.id} pour ${raison}`);
}
module.exports.help = {
    name: "reportb"
}